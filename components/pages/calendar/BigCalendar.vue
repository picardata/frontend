<template>
  <div>
    <base-header class="pb-6 content__title content__title--calendar">
      <div class="row align-items-center py-4">
        ...
        <div class="col-lg-6 mt-3 mt-lg-0 text-lg-right">
          <a href="#" class="fullcalendar-btn-prev btn btn-sm btn-default" @click.prevent="prev">
            <i class="fas fa-angle-left" />
          </a>
          <a href="#" class="fullcalendar-btn-next btn btn-sm btn-default" @click.prevent="next">
    ...
    <modal :show.sync="showEditModal" modal-classes="modal-secondary">
      ...
      <template slot="footer">
        <base-button native-type="submit" type="primary" class="new-event--add" @click="editEvent">
          Update
        </base-button>
...
export default {
  ...
  data () {
    const monthText = (m + 1) > 10 ? (m + 1) : `0${m + 1}`
    const yearAndMonth = `${y}-${monthText}`
    return {
  ...
  methods: {
    ...
    editEvent () {
      const index = this.events.findIndex(e => e.title === this.model.title)
      if (index !== -1) {
